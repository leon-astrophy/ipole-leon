...
void load_par_from_argv(int argc, char *argv[], Params *params) {
  ...
  //my patch, only luminosity? //
  params->only_lum = 1;

  // set default values here, see par.h for descriptions
  params->add_ppm = 0;
  params->qu_conv = 0;
  ...
  // or DSGRA_PC, depends on the AGN
  params->dsource = DM87_PC; 
...
void try_set_parameter(const char *word, const char *value, Params *params) {
  ...
  // my patch, want only luminosity ? //
  set_by_word_val(word, value, "only_luminosity", &(params->only_lum), TYPE_INT);
