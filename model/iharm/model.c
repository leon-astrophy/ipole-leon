//*************************************************************************
//*
//* Routines for loading GRMHD models from iharm3d/other codes
//*
//*************************************************************************
...
// Only for outputting emissivities
#include "model_radiation.h" 

// For interpolating arbitrary grids
#include "simcoords.h"  
...
// use which temperature ratio formula?
...
// if this value > 0, use to renormalize M_unit &c.
double target_mdot = 0.0;  

// MOLECULAR WEIGHTS
// e.g., used for He with 2 protons+neutrons per 2 electrons
static double Ne_factor = 1.;  
...
// flags for electron and radiations
...
// set radiative transfer model parameters
...
// read from dump file, which GRMHD code are we using? //
...
double set_tinterp_ns(double X[NDIM], int *nA, int *nB)
{
  ...
  //////////////////////////////////////////////////////////////////////
  //  TODO, this should really only happen at r >> risco, but still...
  //////////////////////////////////////////////////////////////////////
  //  in slow light, when we reset based on tB, sometimes we overshoot
  if (tinterp < 0.) tinterp = 0.; 
  if (tinterp > 1.) tinterp = 1.; 
  return tinterp;
  #else
  *nA = 0;
  *nB = 0;
...
// get sigma smoothfac (what is this?)
